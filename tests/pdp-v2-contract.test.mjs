...
test('the narrative consumes all editorial slots in open chapters', async () => {
  const narrative = await source('sections/milaura-product-narrative-v2.liquid');

  for (const slot of ['E01', 'E02', 'E03']) {
    assert.match(narrative, new RegExp(`\\[MILAURA:${slot}\\]`));
  }
  assert.match(narrative, /product\.description/);
  assert.doesNotMatch(narrative, /role="tab"/);
  assert.doesNotMatch(narrative, /<details/);
});
