...
test('the rare-pieces collection template uses the verified geode handle', () => {
  ...
  assert.equal(settings.star_label, 'Le produit star');
...
test('homepage and navigation point to the rare-pieces collection route', () => {
  ...
  assert.equal(homepage.sections.hero_homepage.settings.title, 'Certaines pierres méritent une place à part.');
  assert.equal(homepage.sections.hero_homepage.settings.cta_label, 'Découvrir les pièces rares');
  assert.equal(
    homepage.sections.hero_homepage.settings.desktop_asset,
    'milaura-hero-pieces-rares-lapis.webp',
  );
  assert.equal(
...
test('the geode contract is explicit in the implementation', () => {
  ...
  assert.match(section, /section\.settings\.star_label/);
  assert.match(section, /milaura-rare-pieces__star-eyebrow/);
  assert.doesNotMatch(section, /milaura-rare-pieces__star-product-title/);
