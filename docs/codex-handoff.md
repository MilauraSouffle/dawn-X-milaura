...
GO visuel du 2026-09-22 17:30 CEST : Patrice valide explicitement la V3 avec `magnifique j'adore`. Cette composition est maintenant la reference visuelle approuvee. Cela n autorise ni push live ni mutation Admin.

Etat au 2026-09-22 19:40 CEST : Patrice donne le GO pour avancer sur la landing en reutilisant le template Sodalite. Le perimetre canonique est donc `Grenat & cornaline`. `sections/milaura-sodalite-landing.liquid` reste le moteur unique mais son schema est generalise en `Landing de selection`. `templates/collection.selection-automne.json` reprend la video et la composition V3, met le bracelet Iris en produit star, rend neuf autres produits publics, puis place les guides Grenat et Cornaline apres les produits. Les deux references encore en HTTP 404, bracelet grenat 8 mm et puces grenat 8 mm, sont exclues.

La preview privee passe la QA `390 x 844` et `1440 x 900` : un H1, dix produits publics, deux guides, zero overflow, onglets fonctionnels et zero erreur navigateur. Theme Check reste a 0 erreur et 16 avertissements historiques. Pullback exact de la section landing, du template Automne et de la dependance guide pierre. Theme prive : `200259043675`. Preview : `https://milaura-2.myshopify.com/collections/selection-de-karine?view=selection-automne&preview_theme_id=200259043675`.

La route permanente `/collections/selection-automne` renvoie encore 404 parce que la collection Shopify Admin n est pas creee. A 19:40 CEST, la prochaine gate etait le GO visuel Patrice sur la landing, puis la creation Admin de la collection, l affectation du template `selection-automne`, le title SEO, la meta description, le canonical, le sitemap et les recommandations complementaires. Aucun live, Admin, Ads ou Pinterest n a ete modifie dans ce lot. Manifeste : `docs/campaigns/automne/manifest.md`. Checkpoint : `docs/checkpoints/2026-09-22-1940-autumn-landing-private.md`.

GO visuel landing du 2026-09-22 19:53 CEST : Patrice valide explicitement avec `go visuel tout est parfait`. La landing Automne est visuellement approuvee sur le theme prive. La prochaine gate est exclusivement le GO Shopify Admin pour creer la collection permanente, lui affecter le template et configurer ses champs SEO. Aucun GO live, Ads ou Pinterest n est acquis. Checkpoint : `docs/checkpoints/2026-09-22-1953-autumn-landing-visual-go.md`.

GO Admin du 2026-09-22 19:59 CEST : Patrice autorise explicitement l etape Admin avec `GO Admin`. La collection manuelle `681359311195`, handle `selection-automne`, est creee avec dix produits, le title SEO `Bijoux grenat et cornaline : sélection d’automne | MilAura` et une meta description de 150 caracteres. Elle reste non publiee sur `0 canal`; la route publique renvoie donc encore `404` et le live `190430282075` est intact. Le modele Admin reste `Collection par defaut`, car `selection-automne` existe seulement sur le theme prive et ne peut pas etre selectionne avant son deploiement live. Aucun Search and Discovery, Ads ou Pinterest modifie. Prochaine gate : GO live distinct pour pousser le template, l affecter, publier la collection et verifier HTTP 200, canonical, sitemap, schema, panier et tracking. Checkpoint : `docs/checkpoints/2026-09-22-1959-autumn-collection-admin.md`.

```text
Reprends la landing Automne depuis docs/checkpoints/2026-09-22-1959-autumn-collection-admin.md. Le theme prive 200259043675 contient la landing Grenat et Cornaline sur le template collection.selection-automne. La collection Admin 681359311195 existe sous /collections/selection-automne avec dix produits et ses champs SEO, mais reste publiee sur 0 canal avec le modele par defaut. Commence en lecture seule. Ne pousse le template sur le theme public 190430282075, ne l affecte a la collection et ne publie les canaux qu apres un GO live explicite. Les deux produits grenat 8 mm encore en HTTP 404 restent exclus. Search and Discovery, Ads et Pinterest restent des gates distinctes.
```
